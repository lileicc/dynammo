function draw_skel(frame, names)
	if (size(frame, 2) == 3)
		markers = frame;
	else
	...
	plot3(markers(:,1), markers(:,2), markers(:,3), 'ob', 'MarkerSize', 10, 'MarkerFaceColor', [0.2627, 0.2627, 1]);
	axis equal;
	...
	for i = 1:3:size(hier,2)
		...
		if (sum(a) == 1 && sum(b) == 1)
			...
			if (strcmp(hier(i+2), 'r'))
				col = [1 0 0];
                col = [0.5804, 0.2275, 0.2275];
			elseif (strcmp(hier(i+2), 'g'))
				col = [0 1 0];
                col = [0.4431, 0.7490, 0.5843];
			elseif (strcmp(hier(i+2), 'b'))
				col = [0 0 1];
                col = [0.9608, 0.7686, 0.5490];
			end
			%line(data(:,1), data(:,2), data(:,3), 'Color', col);
            line(data(:,1), data(:,2), data(:,3), 'Color', col, 'LineWidth', 4);
		end
	end
end
