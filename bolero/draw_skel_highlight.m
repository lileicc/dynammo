function draw_skel_highlight(frame, names, bones, bonecolors, missing)
	...
	%bonecolors is vector of colors or [nan nan nan], should be same size as bones.
	assert(size(bonecolors,2) == 3);
	assert(size(bonecolors,1) == size(bones,1));
	hold off;
	plot3(markers(~missing,1), markers(~missing,2), markers(~missing,3), '.', 'LineSmoothing', 'off', 'Color', [0 0 0]);
	hold on;
	plot3(markers(missing,1), markers(missing,2), markers(missing,3), 'ob', 'LineSmoothing', 'off');
	hold off;
	axis equal;
...
... %The centered stuff:	
	...
	if nargin == 2
		...
		for i = 1:size(bones,1)
			...
			data = [markers(a,:); markers(b,:)];
			col = neutral;
			if ~isnan(bonecolors(i,1))
				line(data(:,1), data(:,2), data(:,3), 'Color', bonecolors(i,:), 'LineSmoothing', 'off', 'LineWidth', 2);
			else
				line(data(:,1), data(:,2), data(:,3), 'Color', neutral, 'LineSmoothing', 'off');
			end
		end
	end
end
